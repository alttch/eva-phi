...
__help__ = """
PHI for Softron SM-200 Modbus temperature/humidity sensor. Ports: "t", "h"
"""
...
class PHI(GenericPHI):
    ...
    def get(self, port=None, cfg=None, timeout=0):
        ...
        try:
            rr = mb.read_input_registers(30002, 2, unit=self.unit_id)
            mb.release()
            if rr.isError():
                self.log_debug(rr)
                return None
            return {'t': rr.registers[0] / 100, 'h': rr.registers[1] / 100}
        except:
            log_traceback()
            return None
